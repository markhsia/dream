...
def model(X, M, Y, train=False, reuse=False):
    with tf.variable_scope('model', reuse=reuse):
        ...
        clf_h = tf.reshape(clf_h, [-1, 3, n_embd])
        ...
        clf_logits = tf.reshape(clf_logits, [-1, 3])
...
def transform_dream(X1, X2, X3, X4, X5, S1):
    n_batch = len(X1)
    xmb = np.zeros((n_batch, 3, n_ctx, 3), dtype=np.int32)
    mmb = np.zeros((n_batch, 3, n_ctx), dtype=np.float32)
    start = encoder['_start_']
    delimiter = encoder['_delimiter_']
    for i, (x1, x2, x3, x4, x5, s1), in enumerate(zip(X1, X2, X3, X4, X5, S1)):
        x12 = [start]+x1[:max_len]+x2[:max_len]+[delimiter]+x3[:max_len]+[clf_token]
        x13 = [start]+x1[:max_len]+x2[:max_len]+[delimiter]+x4[:max_len]+[clf_token]
        x14 = [start]+x1[:max_len]+x2[:max_len]+[delimiter]+x5[:max_len]+[clf_token]
        l12 = len(x12)
        l13 = len(x13)
        l14 = len(x14)
        s1 = s1[:max_len]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--desc', type=str, default='dream')
    parser.add_argument('--dataset', type=str, default='dream')
    ...
    (trX1, trX2, trX3, trX4, trX5, trY), (vaX1, vaX2, vaX3, vaX4, vaX5, vaY), (teX1, teX2, teX3, teX4, teX5) = encode_dataset(dream(data_dir), encoder=text_encoder)
    ...
    trX1, trS1 = get_speaker(trX1, encoder['_speaker1_'], encoder['_speaker2_'])
    vaX1, vaS1 = get_speaker(vaX1, encoder['_speaker1_'], encoder['_speaker2_'])
    teX1, teS1 = get_speaker(teX1, encoder['_speaker1_'], encoder['_speaker2_'])
    
    n_special = len(encoder)
    max_len = n_ctx//2-2
    n_ctx = min(max([len(x1[:max_len])+len(x2[:max_len])+max(len(x3[:max_len]), len(x4[:max_len]), len(x5[:max_len])) for x1, x2, x3, x4, x5 in zip(trX1, trX2, trX3, trX4, trX5)]+\
                     [len(x1[:max_len])+len(x2[:max_len])+max(len(x3[:max_len]), len(x4[:max_len]), len(x5[:max_len])) for x1, x2, x3, x4, x5 in zip(vaX1, vaX2, vaX3, vaX4, vaX5)]+\
                     [len(x1[:max_len])+len(x2[:max_len])+max(len(x3[:max_len]), len(x4[:max_len]), len(x5[:max_len])) for x1, x2, x3, x4, x5 in zip(teX1, teX2, teX3, teX4, teX5)])+3, n_ctx)

    trX, trM = transform_dream(trX1, trX2, trX3, trX4, trX5, trS1)
    vaX, vaM = transform_dream(vaX1, vaX2, vaX3, vaX4, vaX5, vaS1)

    if submit:
        teX, teM = transform_dream(teX1, teX2, teX3, teX4, teX5, teS1)
    ...
    X_train = tf.placeholder(tf.int32, [n_batch_train, 3, n_ctx, 3])
    M_train = tf.placeholder(tf.float32, [n_batch_train, 3, n_ctx])
    X = tf.placeholder(tf.int32, [None, 3, n_ctx, 3])
    M = tf.placeholder(tf.float32, [None, 3, n_ctx])
